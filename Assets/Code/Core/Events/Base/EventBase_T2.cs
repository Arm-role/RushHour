...
public abstract class EventBase<T1, T2>
{
    private readonly List<WeakListener<(T1, T2)>> _listeners = new();

    public void Subscribe(Action<T1, T2> callback, object owner = null)
    {
        if (callback == null) { Debug.Log("Action Null"); return; }

        Action<(T1, T2)> wrapper = tuple => callback(tuple.Item1, tuple.Item2);

        var listener = new WeakListener<(T1, T2)>(wrapper, owner);
    ...
    public void UnSubscribe(Action<T1, T2> callback)
    {
        ...
        {
            return w.Matches((tuple) => callback(tuple.Item1, tuple.Item2));
        });
    }
...
public abstract class EventBaseParam<T1, T2> : EventBase<T1, T2>
{
    protected T1 _param1 = default;
    protected T2 _param2 = default;

    public override void Invoke(T1 param1, T2 param2)
    {
